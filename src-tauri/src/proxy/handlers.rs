...
use super::{
    ...
    response_processor::{
        body_with_route_completion, create_logged_passthrough_stream, create_usage_collector,
        process_response, read_decoded_body, strip_entity_headers_for_rebuilt_body,
        strip_hop_by_hop_response_headers, usage_logging_enabled, ResponseLifecycle,
        SseUsageCollector,
    },
    server::ProxyState,
...
async fn handle_responses_for_app(
    ...
    let lifecycle = ResponseLifecycle::new(
        result.connection_guard.take(),
        result.codex_route_attempt.take(),
    )
    .for_codex_endpoint(&endpoint);
    ctx.outbound_model = result.outbound_model.take();
    ctx.provider = result.provider;
    let response = result.response;
    ...
    process_response(response, &ctx, &state, &CODEX_PARSER_CONFIG, lifecycle).await
}
...
async fn handle_codex_responses_namespace_restore(
    ...
    if !status.is_success() {
        return process_response(response, ctx, state, &CODEX_PARSER_CONFIG, lifecycle).await;
    }
    ...
    let _connection_guard = lifecycle.connection_guard.take();
    let body_timeout =
        if ctx.app_config.auto_failover_enabled && ctx.app_config.non_streaming_timeout > 0 {
            std::time::Duration::from_secs(ctx.app_config.non_streaming_timeout as u64)
        } else {
    ...
    let (restored_bytes, route_completed) = match serde_json::from_slice::<Value>(&body_bytes) {
        Ok(mut value) => {
            let route_completed = lifecycle.response_completes_route(&value);
            ...
            let restored_bytes = match serde_json::to_vec(&value) {
            ...
            (restored_bytes, route_completed)
        }
    ...
    let route_attempt = if route_completed {
        lifecycle.codex_route_attempt.take()
    } else {
        None
    };
    builder
        .body(body_with_route_completion(restored_bytes, route_attempt))
        .map_err(|e| {
...
async fn handle_codex_chat_to_responses_transform(
    ...
    let route_attempt = lifecycle.take_route_attempt_for_response(&responses_response);
    state
        .codex_chat_history
    ...
    builder
        .body(body_with_route_completion(
...
async fn handle_codex_anthropic_to_responses_transform(
    ...
    if response.is_sse() || (is_stream && !response.is_json()) {
        ...
        return build_codex_anthropic_sse_response(sse_stream, ctx, state, status, lifecycle);
    }
...
mod tests {
    use super::{
        ...
        codex_proxy_error_json, map_proxy_error_to_status, responses_sse_to_response_value,
