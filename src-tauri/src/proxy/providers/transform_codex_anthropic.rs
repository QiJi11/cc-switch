...
pub(crate) fn map_anthropic_stop_reason_to_status(
    ...
    match stop_reason {
        Some("end_turn" | "tool_use" | "stop_sequence") => ("completed", None),
        ...
        Some("pause_turn") => {
            log::warn!("[Codex] Received unexpected Anthropic stop_reason=pause_turn, treating it as incomplete");
            ("incomplete", Some("max_output_tokens"))
        }
        Some(other) => {
            log::warn!(
                "[Codex] Received unknown Anthropic stop_reason={other}, treating it as incomplete"
            );
            ("incomplete", Some("max_output_tokens"))
        }
        None => ("incomplete", Some("max_output_tokens")),
    }
}
...
mod tests {
    ...
    #[test]
    fn only_explicit_success_stop_reasons_complete_anthropic_response() {
        for reason in ["end_turn", "tool_use", "stop_sequence"] {
            assert_eq!(
                map_anthropic_stop_reason_to_status(Some(reason)),
                ("completed", None)
            );
        }
        for reason in [
            ...
            assert_eq!(map_anthropic_stop_reason_to_status(reason).0, "incomplete");
        }
    }
