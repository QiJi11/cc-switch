...
pub(crate) fn chat_completion_to_response_with_context(
    ...
    let (response_status, incomplete_reason) = response_status_from_finish_reason(finish_reason);
...
mod tests {
    ...
    #[test]
    fn only_explicit_success_finish_reasons_complete_chat_response() {
        for reason in ["stop", "tool_calls", "function_call"] {
            assert_eq!(
                response_status_from_finish_reason(Some(reason)),
                ("completed", None)
            );
        }
        for reason in [
            ...
            assert_eq!(response_status_from_finish_reason(reason).0, "incomplete");
        }
    }
