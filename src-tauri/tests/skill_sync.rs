...
#[cfg(unix)]
fn try_symlink_dir(src: &std::path::Path, dest: &std::path::Path) -> bool {
...
fn sync_to_app_removes_disabled_and_orphaned_ssot_symlinks() {
    ...
    if !try_symlink_dir(&disabled_skill, &opencode_skills_dir.join("disabled-skill"))
        || !try_symlink_dir(&orphan_skill, &opencode_skills_dir.join("orphan-skill"))
    {
        return;
    }
